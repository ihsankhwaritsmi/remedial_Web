async function fetching(){
    const apiUrl = "https://pokeapi.co/api/v2/pokemon?limit=35&offset=0";
    let data = '';
    try{
    ...
    for (let i = 0; i < data.results.length; i++) {
        ...
        html += `<div class="card text-center mb-3 m-3 shadow" style="width: 300px; length:900px" id="loop">
                                <div class="card-body">
                                    <p class="card-text" id="poke">${data.results[i].name}</p>
                                    <a href="${data.results[i].url}" class="btn btn-primary" id="btn1">Detail</a>
                                </div>
                            </div>`
